...
class login(QWidget):
    ...
    def initUi(self):
        ...
        self.radioButton = QRadioButton("被测程序输入")
    ...
    def addNum(self):
        ...
        if self.radioButton.isChecked():
            ...
            # print(diy)
        intext = self.inputpath.text()  # 获取文本框内容
        intext = os.path.relpath(intext)
        outtext = self.outputpath.text()
        outtext = os.path.relpath(outtext)
        test = self.exepath.text()
        test = os.path.relpath(test)
        time = self.timeLineEdit.text()
        cb = self.cbLineEdit.text()
        memory = self.memoryLineEdit.text()
        if cb == "master":
